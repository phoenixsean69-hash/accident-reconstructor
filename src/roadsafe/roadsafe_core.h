...
// ROADSAFE_EDITOR_RECORD_FIELDS_V2
// ROADSAFE_DOMAIN_ACTIVE_STATE_V1
struct EvidenceRecord
{
    ...
    // Current native editor parameters.
    // Type-specific fields are intentionally kept on the evidence
    // record so the Properties panel edits the case itself.
    float lengthMeters=0.0f;
    float widthMeters=0.0f;
    float frictionCoefficient=0.70f;
    float fieldRadiusMeters=0.0f;
    int estimatedPieceCount=0;
    int categoryIndex=0;
...
struct VehicleRecord
{
    ...
    Vec3 scenePosition;
    float sceneHeadingDegrees=0.0f;

    // Native reconstruction / physics parameters.
    float massKg=0.0f;
    float wheelbaseMeters=0.0f;
    float cgHeightMeters=0.0f;
    float initialSpeedMetersPerSecond=0.0f;
    float steeringDegrees=0.0f;
    float frictionCoefficient=0.70f;
    float restitution=0.20f;
    float crushDepthMeters=0.0f;
...
// ROADSAFE_PREMIUM_ASSET_REFERENCE_V1
// Persistent reference used by the upcoming native GLB/glTF/PBR loader.
struct AssetReference
{
    ...
    // Renderer preference; actual LOD availability is asset-dependent.
    int lodBias=0;
    ...
    bool arReady=true;
};// ROADSAFE_SCENE_ENTITY_MODEL_V2
...
{
    // Stable RoadSafe scene identity.
    std::string id;

    // Temporary compatibility ID for the current editor shell.
    // IDs 1..9 map to the existing starter scene until the
    // Outliner becomes fully dynamic.
    int legacyId=0;

    SceneEntityKind kind=SceneEntityKind::Environment;

    // Index into the corresponding RoadSafe domain collection:
    // vehicles / evidence / measurements. Environment entities
    // use -1.
    int recordIndex=-1;
...
struct RoadSafeCase
{
    ...
    SceneEntityRecord* findSceneEntity(
        int legacyEntityId)
    {
        for (auto& entity : sceneEntities)
        {
            if (entity.active &&
                entity.legacyId==legacyEntityId)
            {
                return &entity;
            }
        }
    ...
    const SceneEntityRecord* findSceneEntity(
        int legacyEntityId) const
    {
